...
router.get("/", async (req, res) => {
    try {
        ...
        if (gender) {
            query.gender = new RegExp(`^${gender}$`, "i"); // Case-insensitive match
        }
...
router.put("/:id", protect, admin, async (req, res) => {
    try {
        const { name, description, price, discountPrice, countInStock, category, brand, sizes, colors, collections, material,
...
router.get("/best-seller", async (req, res) => {
    try {
        ...
        if (bestSeller) {
            ...
            res.status(404).json({
                message: "no best seller founded"
            })
        }
