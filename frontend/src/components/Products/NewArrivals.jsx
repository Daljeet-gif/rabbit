...
const NewArrivals = () => {
    ...
    useEffect(() => {
        const container = scrollRef.current;
        if (container) {
            container.addEventListener("scroll", updateScrollButtions)
            updateScrollButtions();
            return () => container.removeEventListener("scroll", updateScrollButtions);
        }
    }, [newArrivals])
    return (
        <section className='py-16 px-4 lg:px-0'>

            <div className={`container mx-auto text-center mb-10 relative ${isDragging ? "cursor-grabbing" : "cursor-grab"}`}>
                <h2 className='text-3xl font-bold mb-4'>
                    Explore  New Arrivals
                </h2><p className='text-lg text-gray-600 mb-8 '>

                    Dicover this latest styles straight off the running freshly added to kepp your wardrobe on the vuttin edge of fashion
                </p>
