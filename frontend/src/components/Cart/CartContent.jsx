...
const CartContent = ({ cart, userId, guestId }) => {
    ...
    const handleAddToCart = (productId, delta, quantity, size, color) => {
        ...
        if (newQuantity >= 1) {  // Fixed condition
            dispatch(updateCartItemQuantity({  
                productId, 
                quantity: newQuantity,  // Using newQuantity correctly
                guestId, 
                userId, 
                size, 
                color
        ...
        <div>
            {cart.products.map((product, index) => (
                <div key={index} className='flex items-start justify-between py-4 border-b '>
                    ...
                    <div><p className='font-medium'>$ {product.price.toLocaleString()}</p>
                        <button onClick={() => handleRemoveFromCart(product.productId, product.size, product.color)}>
                            <RiDeleteBin3Line className='h-6 w-6 text-red-600'></RiDeleteBin3Line>
